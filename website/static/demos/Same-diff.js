...
var allKeyBindings = [
      ...
      {"p": "different", "q": "same"} ],
    allTrialOrders = [
      ["blue","red","red","red","blue","red","blue","blue","red","blue","blue","red"],
      ["red","blue","blue","blue","red","blue","red","red","blue","red","red","blue"] ],
    allTextColors = ["blue","red"],
...
var experiment = {
  ...
  // The function that gets called when the sequence is finished.
    ...
    //var AverageRT = Math.average(Math, RT);
    console.log(RTsums);
    console.log(RTsums/12);
    var AverageRT = Math.round(RTsums/12);
    console.log(Accuracysum);
    console.log((Accuracysum/12)*100);
    var PerCor = Math.round((Accuracysum/12)*100);
    //var PerCor = Math.average(Accuracy);
    console.log(PerCor);

    $("#AverageRT").html(AverageRT);
    $("#PerCor").html(PerCor);
  },
  ...
  next: function() {
    ...
    var text = experiment.trials.shift();

    var tcolor = randomElement(allTextColors);

    // Compute the correct answer.
    var realParity = (text == tcolor) ? "same" : "different";
    
    showSlide("stage");
    // Display the number stimulus.
    $("#color").text(text);
    $("#color").css('color', tcolor)
    ...
    var keyPressHandler = function(event) {
      ...
      if (keyCode != 81 && keyCode != 80) {
        ...
        var endTime = (new Date()).getTime(),
            ...
            data = {
              stimulus: [text,tcolor,realParity],
              accuracy: realParity == userParity ? 1 : 0,
              rt: endTime - startTime
            };

            RTsums += parseInt(data.rt,10);
            Accuracysum += Number(data.accuracy);
